...
from modules.retriever import MatchingModel_wd
...
class DataLoader(object):
    def __init__(self, vocabs, filename, batch_size, worddrop=0., max_seq_len=256, addition=True):
        ...
        for line in open(filename).readlines():
            ...
            eq, wd, tgt, tgt_processed = eq.split()[:max_seq_len], wd.split()[:max_seq_len], tgt.split()[
                                                                                             :max_seq_len], tgt_processed.split()[
                                                                                                            :max_seq_len]
            eq_tokens.append(eq)
            wd_tokens.append(wd)
    ...
    def __iter__(self):
        ...
        while cur < len(indices):
            ...
            # else:
            data = [{'eq_tokens': self.eq[i], 'wd_tokens': self.wd[i], 'tgt_tokens': self.tgt[i],
                     'tgt_processed_tokens': self.tgt_processed[i]} for i in indices[cur:cur + self.batch_size]]
            cur += self.batch_size
...
def main(args, local_rank):
    ...
    if args.resume_ckpt:
        model = MatchingModel_wd.from_pretrained(vocabs, args.resume_ckpt)
    else:
        model = MatchingModel_wd.from_params(vocabs, args.layers, args.embed_dim, args.ff_embed_dim, args.num_heads, args.dropout, args.output_dim, args.bow)
    ...
    while global_step <= args.total_train_steps:
        for batch in train_data:
            batch = move_to_device(batch, device)
            loss, acc, bsz = model(batch['eq_tokens'], batch['wd_tokens'], batch['tgt_tokens'], args.label_smoothing)
